...
export function useSpotifyPlayback({
  enabled,
  userId,
}: UseSpotifyPlaybackOptions): SpotifyPlaybackHandle {
  ...
  async function waitForPause(timeoutMs = 1000) {
    if (
      !playerRef.current ||
      typeof playerRef.current.getCurrentState !== "function"
    ) {
      return;
    }
  ...
  const requestPlaybackToken = useCallback(async () => {
    ...
    const response = await fetch(
      `${API_BASE_URL}/playback/token?userId=${encodeURIComponent(userId)}`
    );
    if (!response.ok) {
    ...
    if (!payload.eligible || !payload.accessToken) {
      ...
      if (payload.missingScopes && payload.missingScopes.length > 0) {
        throw new Error(
          `Reauthorize Spotify to grant: ${payload.missingScopes.join(", ")}`
        );
      }
      throw new Error(
        "Playback not available for this account. Try reconnecting Spotify."
      );
    }
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!supported) {
        setError(
          "This browser does not support Spotify Web Playback (DRM/Widevine). Try Chrome or Edge."
        );
        setStatus("error");
        return;
      }
      ...
      const applyState = (state: any) => {
        ...
        if (withinSeekWindow && lastSeekTargetRef.current != null) {
          ...
          if (incomingPos < lowerBound) {
            ...
            if (
              seekAnimTimerRef.current == null &&
              seekAnimBaseRef.current != null &&
              seekAnimStartRef.current != null
            ) {
              seekAnimTimerRef.current = window.setInterval(() => {
                if (
                  !lastSeekAtRef.current ||
                  !lastSeekTargetRef.current ||
                  !seekAnimStartRef.current ||
                  !seekAnimBaseRef.current
                ) {
                  if (seekAnimTimerRef.current) {
                    window.clearInterval(seekAnimTimerRef.current);
                    seekAnimTimerRef.current = null;
                ...
                const nextUiPos = Math.min(
                  base + elapsed,
                  durationMs ?? base + elapsed
                );
        ...
        if (maybeCurrent) {
          const imageUrl =
            ...
            maybeCurrent.album.images.length > 0
              ? maybeCurrent.album.images[0].url
              : null;
          const artist =
          ...
          if (
            ...
            maybeCurrent.id &&
            maybeCurrent.id !== currentTrackIdRef.current
          ) {
            setPositionMs(0);
          }
      ...
      if (!cancelled) {
        setError(
          err instanceof Error ? err.message : "Unable to load Spotify SDK"
        );
        setStatus("error");
      }
    });
  ...
  const play = useCallback(
    async (spotifyTrackId: string) => {
      ...
      const switchingTracks =
        !!activeTrackId && activeTrackId !== spotifyTrackId;
      if (switchingTracks) {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          throw new Error(
            "Spotify needs an active device. Open Spotify on any device and try again."
          );
        }
    ...
    async (nextPositionMs: number) => {
      ...
      try {
        ...
        if (state && typeof state.position === "number") {
          ...
          if (
            lastSeekTargetRef.current != null &&
            state.position >= Math.max(0, lastSeekTargetRef.current - 120)
          ) {
            lastSeekTargetRef.current = null;
            lastSeekAtRef.current = null;
            if (seekAnimTimerRef.current) {
